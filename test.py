...
def pre_selection(X,y,n_folds,best_model):
    ...
    #rf_classifier = RandomForestClassifier(n_estimators=c_a,max_depth=prof,random_state=r_s)
    ...
    sfs = SequentialFeatureSelector(rf_classifier, k_features=15, forward=False,
                                     floating=True, verbose=2, scoring=scorer,
                                     cv=stratified_kfold)
    ...
    rf_classifier.fit(X[selected_features], y)

    # Make predictions with cross-validation
    y_pred =cross_val_predict(rf_classifier,X[selected_features],y,cv=stratified_kfold)
...
# Perform a grid search for the best hyperparameters and return the best estimator
def grid(X,y,n_folds):
    # Random Forest Classifier
    model = RandomForestClassifier(random_state=13)
    # Hyperparameters to tune
    param_grid = {
        'n_estimators': list(range(100, 1000, 100)),  # Number of trees
        'max_depth': [3, 4, 5, 6, 7],  # Depth of each tree. None: no restriction
        'max_features': [None, 'sqrt'],  # Maximum number of features for each tree node
        'min_impurity_decrease': [0, 0.001, 0.01, 0.05, 0.1, 0.3],  # Controls whether a node should split based on impurity decrease
...
#                    Data Reading                                #
##################################################################
file_path = 'Datos/datos_onehot_all.xlsx'
...
if bool_selected_features == False:
    # CROSS VALIDATION
    y=categories.loc[id_adv,'tadlqi_advanced']
    X=data.loc[id_adv,:]
    ...
    plot_importances(importances_instrumental, category='tadlqi_advanced', method=method, path=path)
...
# If sequential feature selection is performed
    ...
    plot_grid_search(grid_search, path, category='tadlqi_advanced')
